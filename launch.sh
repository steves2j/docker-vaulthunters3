...
if ! [[ -f "Vault Hunters 3rd Edition-Update-7.6_Server-files.zip" ]]; then
  rm -fr config mods defaultconfigs scripts forge-*.jar start.sh *Server.zip
	curl -Lo 'Vault Hunters 3rd Edition-Update-7.6_Server-files.zip' 'https://edge.forgecdn.net/files/4394/323/Vault Hunters 3rd Edition-Update-7.6_Server-files.zip' && unzip -u -o 'Vault Hunters 3rd Edition-Update-7.6_Server-files.zip' -d /data
